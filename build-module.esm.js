...
pp$1.parseStatement = function (context, topLevel, exports) {
  ...
  switch (starttype) {
    ...
    default:
      ...
      {return this.parseLabeledStatement(node, maybeName, expr, context);} else
...
pp$1.parseBreakContinueStatement = function (node, keyword) {
  ...
  this.next();
  if (this.eat(types.semi) || this.insertSemicolon()) {node.label = null;} else
  if (this.type !== types.name) {this.unexpected();} else
  {
...
pp$1.parseReturnStatement = function (node) {
  ...
  if (this.eat(types.semi) || this.insertSemicolon()) {node.argument = null;} else
...
pp$1.parseExport = function (node, exports) {
  ...
  if (this.shouldParseExportStatement()) {
    ...
    {this.checkVariableExport(exports, node.declaration.declarations);} else
...
pp$1.checkPatternExport = function (exports, pat) {
  var type = pat.type;
  if (type === "Identifier")
  {this.checkExport(exports, pat.name, pat.start);} else
  ...
  if (type === "Property")
  {this.checkPatternExport(exports, pat.value);} else
  if (type === "AssignmentPattern")
  {this.checkPatternExport(exports, pat.left);} else
  if (type === "RestElement")
  {this.checkPatternExport(exports, pat.argument);} else
...
pp$3.parseMaybeUnary = function (refDestructuringErrors, sawUnary) {
  ...
  if (this.isContextual("await") && (this.inAsync || !this.inFunction && this.options.allowAwaitOutsideFunction)) {
    ...
    this.checkExpressionErrors(refDestructuringErrors, true);
    if (update) {this.checkLVal(node.argument);} else
    if (this.strict && node.operator === "delete" &&
    node.argument.type === "Identifier")
    {this.raiseRecoverable(node.start, "Deleting local variable in strict mode");} else
    {sawUnary = true;}
  ...
  {return this.buildBinary(startPos, startLoc, expr, this.parseMaybeUnary(null, false), "**", false);} else
...
pp$3.parseNew = function () {
  ...
  if (this.eat(types.parenL)) {node.arguments = this.parseExprList(types.parenR, this.options.ecmaVersion >= 8, false);} else
...
pp$3.parsePropertyValue = function (prop, isPattern, isGenerator, isAsync, startPos, startLoc, refDestructuringErrors, containsEsc) {
  ...
  this.type !== types.comma && this.type !== types.braceR && this.type !== types.eq) {
    ...
    if (prop.value.params.length !== paramCount) {
      ...
      if (prop.kind === "get")
      {this.raiseRecoverable(start, "getter should have no params");} else
...
pp$5.declareName = function (name, bindingType, pos) {
  ...
  if (bindingType === BIND_LEXICAL) {
    ...
    {redeclared = scope$2.lexical.indexOf(name) > -1;} else
...
pp$7.updateContext = function (prevType) {
  ...
  {this.exprAllowed = false;} else
  if (update = type.updateContext)
  {update.call(this, prevType);} else
...
types._function.updateContext = types._class.updateContext = function (prevType) {
  ...
  {this.context.push(types$1.f_expr);} else
...
types.star.updateContext = function (prevType) {
  if (prevType === types._function) {
    ...
    {this.context[index] = types$1.f_expr_gen;} else
...
pp$9.nextToken = function () {
  ...
  if (curContext.override) {return curContext.override(this);} else
...
pp$9.readRegexp = function () {
  ...
  for (;;) {
    ...
    if (!escaped) {
      if (ch === "[") {inClass = true;} else
      if (ch === "]" && inClass) {inClass = false;} else
      if (ch === "/" && !inClass) {break;}
      escaped = ch === "\\";
    } else {escaped = false;}
    ++this.pos;
...
var prototypeAccessors$1 = { inFunction: { configurable: true }, inGenerator: { configurable: true }, inAsync: { configurable: true }, allowSuper: { configurable: true }, allowDirectSuper: { configurable: true }, treatFunctionsAsVar: { configurable: true }, inNonArrowFunction: { configurable: true } };
...
prototypeAccessors$1.inGenerator.get = function () {return (this.currentVarScope().flags & SCOPE_GENERATOR$1) > 0;};
prototypeAccessors$1.inAsync.get = function () {return (this.currentVarScope().flags & SCOPE_ASYNC$1) > 0;};
prototypeAccessors$1.allowSuper.get = function () {return (this.currentThisScope().flags & SCOPE_SUPER$1) > 0;};
...
prototypeAccessors$1.inNonArrowFunction.get = function () {return (this.currentThisScope().flags & SCOPE_FUNCTION$1) > 0;};
...
pp$1$1.isLet = function (context) {
  ...
  if (isIdentifierStart$1(nextCh, true)) {
    var pos = next + 1;
    while (isIdentifierChar$1(this.input.charCodeAt(pos), true)) {++pos;}
    var ident = this.input.slice(next, pos);
...
pp$1$1.isAsyncFunction = function () {
  ...
  this.input.slice(next, next + 8) === "function" && (
  next + 8 === this.input.length || !isIdentifierChar$1(this.input.charAt(next + 8)));
};
...
pp$1$1.parseBreakContinueStatement = function (node, keyword) {
  var isBreak = keyword === "break";
  this.next();
  if (this.eat(types$2.semi) || this.insertSemicolon()) {node.label = null;} else
  if (this.type !== types$2.name) {this.unexpected();} else
  {
    node.label = this.parseIdent();
    this.semicolon();
  }
...
pp$1$1.parseReturnStatement = function (node) {
  ...
  if (this.eat(types$2.semi) || this.insertSemicolon()) {node.argument = null;} else
  {node.argument = this.parseExpression();this.semicolon();}
  return this.finishNode(node, "ReturnStatement");
};
...
pp$1$1.parseClassElement = function (constructorAllowsSuper) {
  ...
  var tryContextual = function (k, noLineBreak) {
    ...
    var start = this$1.start,startLoc = this$1.startLoc;
    if (!this$1.eatContextual(k)) {return false;}
    if (this$1.type !== types$2.parenL && (!noLineBreak || !this$1.canInsertSemicolon())) {return true;}
    if (method.key) {this$1.unexpected();}
    method.computed = false;
    method.key = this$1.startNodeAt(start, startLoc);
    method.key.name = k;
...
pp$3$1.parseExpression = function (noIn, refDestructuringErrors) {
  ...
  if (this.type === types$2.comma) {
    var node = this.startNodeAt(startPos, startLoc);
    node.expressions = [expr];
    while (this.eat(types$2.comma)) {node.expressions.push(this.parseMaybeAssign(noIn, refDestructuringErrors));}
    return this.finishNode(node, "SequenceExpression");
  }
  return expr;
};
...
pp$3$1.parseMaybeAssign = function (noIn, refDestructuringErrors, afterLeftParse) {
  ...
  if (this.type.isAssign) {
    ...
    {refDestructuringErrors.shorthandAssign = -1;} // reset because shorthand default was used correctly
    if (this.type === types$2.eq)
    {this.checkLValPattern(left);} else
...
pp$3$1.parseMaybeUnary = function (refDestructuringErrors, sawUnary) {
  ...
  if (this.isContextual("await") && (this.inAsync || !this.inFunction && this.options.allowAwaitOutsideFunction)) {
    ...
    node.argument = this.parseMaybeUnary(null, true);
    this.checkExpressionErrors(refDestructuringErrors, true);
    if (update) {this.checkLValSimple(node.argument);} else
    if (this.strict && node.operator === "delete" &&
    node.argument.type === "Identifier")
    {this.raiseRecoverable(node.start, "Deleting local variable in strict mode");} else
    {sawUnary = true;}
...
pp$3$1.parseSubscript = function (base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained) {
  ...
  if (computed || optional && this.type !== types$2.parenL && this.type !== types$2.backQuote || this.eat(types$2.dot)) {
    ...
    node.property = computed ? this.parseExpression() : this.parseIdent(this.options.allowReserved !== "never");
    node.computed = !!computed;
    if (computed) {this.expect(types$2.bracketR);}
    if (optionalSupported) {
      node.optional = optional;
    }
    base = this.finishNode(node, "MemberExpression");
...
pp$3$1.parseNew = function () {
  ...
  if (this.eat(types$2.parenL)) {node.arguments = this.parseExprList(types$2.parenR, this.options.ecmaVersion >= 8, false);} else
...
types$2._function.updateContext = types$2._class.updateContext = function (prevType) {
  ...
  {this.context.push(types$1$1.f_expr);} else
...
types$2.star.updateContext = function (prevType) {
  if (prevType === types$2._function) {
    ...
    {this.context[index] = types$1$1.f_expr_gen;} else
...
var ecma12BinaryProperties = ecma11BinaryProperties$1 + " EBase EComp EMod EPres ExtPict";
...
var ecma12ScriptValues = ecma11ScriptValues$1 + " Chorasmian Chrs Diak Dives_Akuru Khitan_Small_Script Kits Yezi Yezidi";
...
var require$$0 = /*#__PURE__*/Object.freeze( /*#__PURE__*/Object.assign( /*#__PURE__*/Object.create(null), acorn, {
  'default': acorn__default }));
    ...
    return class extends Parser {
      ...
      updateContext(prevType) {
        if (this.type == tt.braceL) {
          var curContext = this.curContext();
          if (curContext == tc_oTag) this.context.push(tokContexts.b_expr);else
          if (curContext == tc_expr) this.context.push(tokContexts.b_tmpl);else
...
TokenTranslator.prototype = {
  ...
  translate(token, extra) {
    ...
    if (type === tt.name) {
      token.type = Token$2.Identifier;

      // TODO: See if this is an Acorn bug
      if (token.value === "static") {
        token.type = Token$2.Keyword;
      }
    ...
    type.isAssign) {
      ...
      token.type = Token$2.Punctuator;
      token.value = this._code.slice(token.start, token.end);
    } else if (type === tt.jsxName) {
      token.type = Token$2.JSXIdentifier;
    } else if (type.label === "jsxText" || type === tt.jsxAttrValueToken) {
      token.type = Token$2.JSXText;
    } else if (type.keyword) {
      if (type.keyword === "true" || type.keyword === "false") {
        token.type = Token$2.Boolean;
      } else if (type.keyword === "null") {
        token.type = Token$2.Null;
      } else {
        token.type = Token$2.Keyword;
      }
    } else if (type === tt.num) {
      token.type = Token$2.Numeric;
      token.value = this._code.slice(token.start, token.end);
      ...
      if (extra.jsxAttrValueToken) {
        ...
        token.type = Token$2.String;
      }

      token.value = this._code.slice(token.start, token.end);
    } else if (type === tt.regexp) {
      token.type = Token$2.RegularExpression;
...
var espree = () => Parser => {
  ...
  return class Espree extends Parser {
    constructor(opts, code) {
      ...
      const options = normalizeOptions$1(opts);
      const ecmaFeatures = options.ecmaFeatures || {};
      const tokenTranslator$1 =
      options.tokens === true ?
...
var require$$1 = getCjsExportFromNamespace$1(_package$1);
...
const parsers = {
  ...
  get regular() {
    if (this._regular === null) {
      this._regular = Parser.extend(espree());
    }
  ...
  get jsx() {
    if (this._jsx === null) {
      this._jsx = Parser.extend(acornJsx(), espree());
    }
    return this._jsx;
  },
...
var version$1$1 = require$$1.version;
